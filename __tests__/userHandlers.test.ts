...
async function request(
    ...
    try {
        ...
        const response = await fetch(
            `http://127.0.0.1:${address.port}/api/users/alice`,
            {
                method,
...
function setup(viewer: Viewer | null = { userId: 'alice', role: Roles.USER }) {
    ...
    return {
        ...
        writes,
        errors,
        handler: createUserHandler(dependencies),
    }
}
...
test.each(['GET', 'PUT', 'DELETE'])(
    'anonymous %s cannot access accounts',
    async method => {
        const state = setup(null)
        expect(
            (
                await request(state.handler, {
                    method,
                    body: method === 'PUT' ? { name: 'Changed' } : undefined,
                })
            ).status
        ).toBe(401)
        expect(state.writes).toEqual([])
...
test.each(['GET', 'PUT', 'DELETE'])(
    'ordinary user cannot %s another account',
    async method => {
        const state = setup()
        expect(
            (
...
test.each([
    ...
    { name: '' },
    {},
])('rejects unsafe profile changes %j', async body => {
    const state = setup()
    expect((await request(state.handler, { method: 'PUT', body })).status).toBe(
        400
    )
    expect(state.writes).toEqual([])
})
    ...
    expect(
        (
            await request(state.handler, {
                method: 'PUT',
                query: { id: 'bob' },
                body: { role: Roles.ADMIN, team: Teams.LEADERSHIP },
            })
        ).status
    ).toBe(200)
...
test('only an administrator can delete a different account', async () => {
    const state = setup({ userId: 'alice', role: Roles.ADMIN })
    expect((await request(state.handler, { method: 'DELETE' })).status).toBe(
        400
    )
    expect(
        (
...
test('unknown methods, invalid identifiers and missing users are explicit errors', async () => {
    const state = setup({ userId: 'alice', role: Roles.ADMIN })
    expect((await request(state.handler, { method: 'POST' })).allow).toBe(
        'GET, PUT, DELETE'
    )
    expect(
        (await request(state.handler, { query: { id: ['alice', 'bob'] } }))
            .status
    ).toBe(400)
    expect(
        (await request(state.handler, { query: { id: 'missing' } })).status
    ).toBe(404)
})
test('repository failures do not expose internal data', async () => {
    const state = setup()
    state.repository.find = async () => {
...
test('user list is restricted and rejects mutations', async () => {
    ...
    expect((await request(createUsersHandler(admin))).text).toContain(
        'bob@example.test'
    )
})
test('answer history checks ownership before querying', async () => {
    const calls: string[] = []
    const state = setup()
    const handler = createAnswerMetricsHandler({
        ...state,
        getAnswers: async userId => {
            calls.push(userId)
            return [{ playerId: userId, completed: 2, date: '09/17/2026' }]
        },
    })
    expect((await request(handler, { query: { user: 'bob' } })).status).toBe(
        403
    )
    expect(
        (await request(handler, { method: 'POST', query: { user: 'alice' } }))
            .status
    ).toBe(405)
    expect(calls).toEqual([])
    expect((await request(handler, { query: { user: 'alice' } })).status).toBe(
        200
    )
    expect(calls).toEqual(['alice'])
})

test('demo administrator can only view and rename its own account', async () => {
    const state = setup({ userId: 'alice', role: Roles.ADMIN, isVisitor: true })
    ...
    const response = await request(createUsersHandler(state))
    expect(JSON.parse(response.text)).toHaveLength(1)
    expect(response.text).not.toContain('bob@example.test')
    const metrics = createAnswerMetricsHandler({
        ...state,
        getAnswers: async () => {
            throw new Error('Must not query another visitor')
        },
    })
    expect((await request(metrics, { query: { user: 'bob' } })).status).toBe(
        403
    )
})
